...
export class DataService {
  ...
  getTotal(id: number): number {
    let dat = this.getData(id);
    return dat.tasks.length;
  }
  ...
  getData(id: number): UserData {
    ...
    for (let i = 0; i < dat.tasks.length; i++) {
      if (dat.tasks[i].id == 0) dat.tasks[i].id = i + 1;
    }
      ...
      for (let i = 0; i < Task.length; i++) {
        if (Task[i].id === id) {
          for (let j = 0; j < Task[i].tasks.length; j++)
            if (Task[i].tasks[j].id === data.id) Task[i].tasks[j] = data;
        }
      }
      localStorage.setItem('Data', JSON.stringify(Task));
    }
